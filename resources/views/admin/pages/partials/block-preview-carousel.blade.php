        ...
        @foreach($slides as $i => $slide)
            <div style="flex:0 0 auto; width:120px; border:1px solid #dee2e6; border-radius:4px; overflow:hidden;">
                @if(!empty($slide['image_url']))
                    <img src="{{ $slide['image_url'] }}" alt="{{ $slide['caption'] ?? '' }}" style="width:100%; height:80px; object-fit:cover;">
                @else
                ...
                @if(!empty($slide['caption']))
                    <div style="padding:4px;"><small>{{ \Illuminate\Support\Str::limit($slide['caption'], 30) }}</small></div>
                @endif
            </div>
        @endforeach
    </div>
