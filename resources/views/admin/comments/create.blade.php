...
<div class="card">
    <div class="card-header">
        {{ trans('vela::global.create') }} {{ trans('vela::cruds.comment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("vela.admin.comments.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(\VelaBuild\Core\Models\Comment::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'visible') === (string) $key ? 'selected' : '' }}>{{ trans('vela::global.status_' . $key) }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ipaddress') ? 'is-invalid' : '' }}" type="text" name="ipaddress" id="ipaddress" value="{{ old('ipaddress', '') }}">
