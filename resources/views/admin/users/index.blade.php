@extends('vela::layouts.admin')
@section('content')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            @can('user_create')
            <a class="btn btn-success" href="{{ route('vela.admin.users.create') }}">
                {{ trans('vela::global.add') }} {{ trans('vela::cruds.user.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('vela::global.app_csvImport') }}
            </button>
            @include('vela::csvImport.modal', ['model' => 'VelaUser', 'route' => 'vela.admin.users.parseCsvImport'])
            @endcan
            @can('role_access')
            <a class="btn btn-info" href="{{ route('vela.admin.roles.index') }}">
                <i class="fas fa-user-tag"></i> {{ trans('vela::cruds.role.title') }}
            </a>
            @endcan
        </div>
    </div>
<div class="card">
    <div class="card-header">
        {{ trans('vela::cruds.user.title_singular') }} {{ trans('vela::global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-User">
            <thead>
                <tr>
                    <th width="10">

                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.id') }}
                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.name') }}
                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.email') }}
                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.email_verified_at') }}
                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.two_factor') }}
                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.roles') }}
                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.last_login_at') }}
                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.profile_pic') }}
                    </th>
                    <th>
                        {{ trans('vela::cruds.user.fields.subscribe_newsletter') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
                </tr>
            </thead>
        </table>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('user_delete')
  let deleteButtonTrans = '{{ trans('vela::global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('vela.admin.users.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).data(), function (entry) {
          return entry.id
      });

      if (ids.length === 0) {
        alert('{{ trans('vela::global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('vela::global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  let dtOverrideGlobals = {
    buttons: dtButtons,
    processing: true,
    serverSide: true,
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('vela.admin.users.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'name', name: 'name' },
{ data: 'email', name: 'email' },
{ data: 'email_verified_at', name: 'email_verified_at' },
{ data: 'two_factor', name: 'two_factor' },
{ data: 'roles', name: 'roles.title' },
{ data: 'last_login_at', name: 'last_login_at' },
{ data: 'profile_pic', name: 'profile_pic', sortable: false, searchable: false },
{ data: 'subscribe_newsletter', name: 'subscribe_newsletter' },
{ data: 'actions', name: '{{ trans('vela::global.actions') }}' }
    ],
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  };
  let table = $('.datatable-User').DataTable(dtOverrideGlobals);
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });

});

</script>
@endsection
