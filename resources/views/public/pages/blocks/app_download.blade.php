@php
    ...
    $s           = \VelaBuild\Core\Services\Blocks\AppDownload::settings($block->settings ?? []);
    ...
    // The block's own link when it has one, else the site's in Settings.
    $links      = \VelaBuild\Core\Services\Blocks\AppDownload::links($content);
    $iosUrl     = $links['ios'];
    $androidUrl = $links['android'];
    $look       = \VelaBuild\Core\Services\Blocks\AppDownload::look($s, $image !== '');
@endphp
{{-- Nothing for a visitor until there is a store link, on the block or in
     Settings → Native App; the page editor says so on the block. --}}
@if($iosUrl !== '' || $androidUrl !== '')
<div class="{{ implode(' ', $look['classes']) }}" style="{{ $look['style'] }}">
    <div class="block-app-download-inner">
        <div class="block-app-download-copy">
            <div class="block-app-download-words">
@if($eyebrow !== '')
                <div class="block-app-download-eyebrow">{{ $eyebrow }}</div>
@endif
@if($heading !== '')
                <h2 class="block-app-download-heading">{{ $heading }}</h2>
@endif
@if($description !== '')
                <p class="block-app-download-description">{{ $description }}</p>
@endif
            </div>
            <div class="block-app-download-side">
...
@if($note !== '')
                <div class="block-app-download-note">{{ $note }}</div>
@endif
            </div>
        </div>
@if($image !== '')
        <div class="block-app-download-media">
            {!! vela_image($image, $imageAlt, [320, 640, 960], 'fit', ['class' => 'block-app-download-img', 'sizes' => '(max-width: 768px) 80vw, 420px']) !!}
        </div>
@endif
    </div>
</div>
