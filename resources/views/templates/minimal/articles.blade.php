...
<section class="mn-section">
    <div class="mn-container">
        @if($posts->count() > 0)
            <p style="color:#757575;margin-bottom:24px;">{{ __('vela::public.showing_articles', ['count' => $posts->count()]) }}</p>
...
<section class="mn-section mn-section-alt">
    <div class="mn-container">
        <div class="mn-section-header" style="text-align:center;">
            <h2>{{ __('vela::public.browse_by_category') }}</h2>
            <p>{{ __('vela::public.find_articles_by_interests') }}</p>
        </div>
        <div class="mn-chips" style="justify-content:center;">
            @foreach($categories as $category)
                <a href="{{ route('vela.public.categories.show', Str::slug($category->name)) }}" class="mn-chip">
                    @if($category->image)
                        {!! vela_image($category->image->url, $category->name, [200, 400, 800], 'crop', []) !!}
                    @endif
                    <span>{{ $category->translated_name }}</span>
                    <span class="mn-chip-count">{{ $category->contents()->where('status', 'published')->count() }}</span>
                </a>
            @endforeach
        </div>
    </div>
