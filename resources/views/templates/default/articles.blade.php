...
<section class="bg-gray-900 text-white py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center">
            <h1 class="text-4xl md:text-5xl font-bold mb-4">{{ __('vela::public.all_articles') }}</h1>
            <p class="text-xl text-gray-300 max-w-2xl mx-auto">
                {{ __('vela::public.all_articles_description') }}
            </p>
        </div>
    </div>
</section>
        ...
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-4">
                ...
                <li class="flex items-center">
                    ...
                    <span class="ml-4 text-gray-900 font-medium">{{ __('vela::public.all_articles') }}</span>
                </li>
            </ol>
        </nav>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex justify-between items-center">
            <p class="text-lg text-gray-600">
                {{ __('vela::public.showing_articles', ['count' => $posts->count()]) }}
            </p>
            <div class="flex items-center space-x-4">
                <a href="{{ route('vela.public.categories.index') }}" class="text-blue-600 hover:text-blue-800 font-semibold">
                    {{ __('vela::public.browse_by_category') }}
                </a>
            </div>
        </div>
...
<section class="py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
            <h2 class="text-3xl font-bold text-gray-900 mb-4">{{ __('vela::public.browse_by_category') }}</h2>
            <p class="text-lg text-gray-600">
                {{ __('vela::public.find_articles_by_interests') }}
            </p>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach($categories as $category)
            <a href="{{ route('vela.public.categories.show', Str::slug($category->name)) }}" class="group bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition duration-300">
                <div class="text-center">
                    @if($category->image)
                    <div class="w-16 h-16 mx-auto mb-4 rounded-full overflow-hidden">
                        {!! vela_image($category->image->url, $category->name, [200, 400, 800], 'crop', ['class' => 'w-full h-full object-cover']) !!}
                    </div>
                    @else
