...
<div class="dk-section-sm dk-section-alt">
    <div class="dk-container">
        <div class="dk-chips">
            <a href="{{ route('vela.public.posts.index') }}" class="dk-chip {{ !request('category') ? 'dk-chip-active' : '' }}">{{ __('vela::public.all') }}</a>
            @foreach($categories as $category)
                <a href="{{ route('vela.public.categories.show', Str::slug($category->name)) }}" class="dk-chip">
                    {{ $category->translated_name }}
                    <span style="font-size:0.75rem;">({{ $category->contents()->where('status', 'published')->count() }})</span>
                </a>
            @endforeach
        </div>
    </div>
    ...
    <div class="dk-container">
        @if($posts->count() > 0)
            <div class="dk-grid dk-grid-3">
                ...
                <a href="{{ route('vela.public.posts.show', $post->slug) }}" class="dk-card" style="text-decoration:none;color:inherit;">
                    ...
                    <div class="dk-card-body">
                        @if($post->categories->count() > 0)
                            <span class="dk-card-category">{{ $post->categories->first()->translated_name }}</span>
                        @endif
                        <h2 class="dk-card-title">{{ $post->translated_title }}</h2>
                        @if($post->translated_description)
                            <p class="dk-card-excerpt">{{ Str::limit($post->translated_description, 110) }}</p>
                        @endif
                        <div class="dk-card-meta">
                            <time datetime="{{ ($post->published_at ?? $post->created_at)->toDateString() }}">{{ ($post->published_at ?? $post->created_at)->format('M j, Y') }}</time>
                        </div>
                    </div>
                </a>
                @endforeach
        ...
        @else
            <div class="dk-empty-state">
                ...
                <a href="{{ route('vela.public.home') }}" class="dk-btn dk-btn-primary">{{ __('vela::public.back_to_home') }}</a>
            </div>
        @endif
    </div>
</section>
