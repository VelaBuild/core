...
<div class="dk-breadcrumb">
    <div class="dk-container">
        ...
        <span class="dk-breadcrumb-sep">/</span>
...
<article>
    <div class="dk-container">
        <div class="dk-article-layout">
            <!-- Main content -->
            <div>
                <header class="dk-article-header">
                    @if($post->categories->count() > 0)
                        <a href="{{ route('vela.public.categories.show', Str::slug($post->categories->first()->name)) }}" class="dk-chip dk-chip-accent">{{ $post->categories->first()->translated_name }}</a>
                    @endif
                ...
                @if($post->main_image)
                <div class="dk-article-featured-img">
                    {!! vela_image($post->main_image->url, $post->translated_title, [600, 1200, 1920], 'fit', ['style' => 'width:100%;border-radius:8px;']) !!}
                </div>
                @endif
                    ...
                    @if($post->translated_content)
                        ...
                        @if(isset($content['blocks']) && is_array($content['blocks']))
                            @foreach($content['blocks'] as $block)
                                @switch($block['type'])
                                    @case('paragraph')
                                        <p>{!! renderDarkMarkdown($block['data']['text'] ?? '') !!}</p>
                                        @break
                                    @case('header')
                                        @php $level = $block['data']['level'] ?? 2; $tag = 'h' . $level; @endphp
                                        <{{ $tag }}>{!! renderDarkMarkdown($block['data']['text'] ?? '') !!}</{{ $tag }}>
                                        @break
                                    @case('list')
                                        @if(isset($block['data']['items']) && is_array($block['data']['items']))
                                            <ul>
                                                @foreach($block['data']['items'] as $item)
                                                    <li>{!! renderDarkMarkdown($item) !!}</li>
                                                @endforeach
                                            </ul>
                                        @endif
                                        @break
                            ...
                            @endforeach
                        @else
                            <p style="color:#a3a3a3;">{{ trans('vela::global.content_processing') }}</p>
                        @endif
                    @else
                        <p style="color:#a3a3a3;">{{ __('vela::public.no_content_available') }}</p>
                    @endif
                    ...
                    <div style="margin-top:40px;">
                        ...
                        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px;margin-top:16px;">
                            @foreach($post->content_images as $image)
                                {!! vela_image($image->url, $post->translated_title, [300, 600], 'fit', ['style' => 'border-radius:8px;width:100%;height:160px;object-fit:cover;']) !!}
                            @endforeach
                        </div>
                    </div>
                    @endif
                ...
                <!-- Share -->
                <div class="dk-share">
                    <span class="dk-share-label">{{ __('vela::public.share') }}:</span>
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->url()) }}" target="_blank" rel="noopener" class="dk-share-btn">{{ __('vela::public.share_on_facebook') }}</a>
                    <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($post->translated_title) }}" target="_blank" rel="noopener" class="dk-share-btn">{{ __('vela::public.share_on_twitter') }}</a>
                </div>
            </div>
            ...
            <aside class="dk-sidebar">
                @if($post->categories->count() > 0)
                <div class="dk-sidebar-widget">
                    <h4>{{ __('vela::public.topics') }}</h4>
                    <div class="dk-chips" style="flex-wrap:wrap;">
                        @foreach($post->categories as $cat)
                            <a href="{{ route('vela.public.categories.show', Str::slug($cat->name)) }}" class="dk-chip">{{ $cat->translated_name }}</a>
                        @endforeach
                    </div>
                </div>
                @endif
...
<section class="dk-related">
    <div class="dk-container">
        ...
        <div class="dk-grid dk-grid-3">
            ...
            <a href="{{ route('vela.public.posts.show', $relatedPost->slug) }}" class="dk-card" style="text-decoration:none;color:inherit;">
                ...
                <div class="dk-card-body">
                    @if($relatedPost->categories->count() > 0)
                        <span class="dk-card-category">{{ $relatedPost->categories->first()->translated_name }}</span>
                    @endif
